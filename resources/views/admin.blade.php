        ...
        <div class="card border-0 shadow-lg admin-card" style="border-radius: 20px; overflow: hidden;">
            ...
            @else
                ...
                <div class="card-body p-5">
                    <form action="{{ $mode === 'edit' ? url('/admin/rooms/' . $room->id) : url('/admin/rooms') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @if($mode === 'edit')
                            @method('PUT')
                        @endif
                        ...
                        <div class="row g-4">
                            ...
                            <div class="col-md-12">
                                <label class="form-label" style="color: var(--current-text-light);">Room Image</label>
                                <input type="file" name="image" id="imageInput" class="form-control" accept="image/*" style="background: var(--current-primary); border: 1px solid var(--current-accent); color: var(--current-text);" {{ $mode === 'create' ? 'required' : '' }}>
                                <small class="text-muted d-block mt-1">Recommended size: 1200x800px. Max size: 2MB.</small>
                                
                                <div id="imagePreview" class="mt-3 {{ $mode === 'edit' && $room->image_path ? '' : 'd-none' }}">
                                    <label class="form-label small" style="color: var(--current-text-light);">Preview:</label>
                                    <img src="{{ $mode === 'edit' ? $room->image_path : '' }}" id="previewImg" class="img-fluid rounded shadow-sm" style="max-height: 200px; object-fit: cover; border: 1px solid var(--current-accent);">
                                </div>
                            </div>
        ...
        if (imageInput) {
            imageInput.addEventListener('change', function() {
                ...
                if (file) {
                    ...
                    // If no file selected (and not in edit mode with existing image), hide preview
