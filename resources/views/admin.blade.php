...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Close Logout Modal (using same close buttons class)
        // Note: The existing closeBtns logic will handle this if we add the event listener to the new modal's buttons too.
        // But since we select all .close-modal-btn, it should work automatically for the new modal too if we re-query or delegate.
        // Let's just add specific listeners for the logout modal to be safe and simple.
        
        const closeLogoutBtns = logoutModal.querySelectorAll('.close-modal-btn, .cancel-btn, .modal-overlay');
